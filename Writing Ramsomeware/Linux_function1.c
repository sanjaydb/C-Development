Linux Code , Finding SUID bit set files , running readelf -d against it to search RPATH, which may contain ".:/" to exploit
readelf -d output if it contains "(NEEDED)" or "(RUNPATH)":
